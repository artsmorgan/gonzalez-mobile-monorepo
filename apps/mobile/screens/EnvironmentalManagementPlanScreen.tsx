...
import {
  createEnvironmentalManagementPlan,
  updateEnvironmentalManagementPlan,
  deleteEnvironmentalManagementPlan,
  listEnvironmentalManagementPlanByCorpo,
} from '@/hooks/evaluationFunctions';
import { eventBus } from '@/hooks/eventBus';

type EnvironmentalManagementPlanScreenNavigationProp = NativeStackNavigationProp<RootStackParamList, 'EnvironmentalManagementPlan'>;
...
const SECCIONES_PRINCIPALES = [
  ...
  { key: 'metodologia', label: 'Metodología de trabajo por ámbitos de acción' },
  { key: 'rol_horario', label: 'Rol y horario de trabajo' },
  { key: 'uniformes', label: 'Uniformes' },
  { key: 'equipos', label: 'Equipos' },
  { key: 'accesorios_varios', label: 'Accesorios varios' },
  { key: 'tiempo_respuesta', label: 'Tiempo de respuesta y disposición de imprevistos' },
  { key: 'distribucion_labores', label: 'Distribución diaria de labores del personal' },
  { key: 'frecuencia_limpieza', label: 'Frecuencia mínima de limpieza de áreas' },
  { key: 'supervision', label: 'Supervisión, método y rol de visitas' },
  { key: 'estrategia', label: 'Estrategia para el adecuado y continuo servicio' },
  { key: 'responsable', label: 'Responsable general del contrato' },
  { key: 'formularios', label: 'Formularios y registro de control de puestos y equipos' },
...
interface EnvironmentalManagementPlan {
  id: string;
  id_local: string;
...
interface EditingEnvironmentalManagementPlan {
  id: string | null;
  id_local: string;
  ubicacion: string;
  objetivo: SeccionElement[];
  metodologia: SeccionElement[];
  rol_horario: SeccionElement[];
  uniformes: SeccionElement[];
  equipos: SeccionElement[];
  accesorios_varios: SeccionElement[];
  tiempo_respuesta: SeccionElement[];
  distribucion_labores: SeccionElement[];
  frecuencia_limpieza: SeccionElement[];
  supervision: SeccionElement[];
  estrategia: SeccionElement[];
  responsable: SeccionElement[];
  formularios: SeccionElement[];
...
export default function EnvironmentalManagementPlanScreen() {
  const { employee, refreshAccessToken, logout } = useAuth();
  const [isMenuVisible, setIsMenuVisible] = useState(false);
  const navigation = useNavigation<EnvironmentalManagementPlanScreenNavigationProp>();

  // Data states
  const [plans, setPlans] = useState<EnvironmentalManagementPlan[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [hasCurrentMarca, setHasCurrentMarca] = useState<boolean>(false);

  // Editing state
  const [editingRecord, setEditingRecord] = useState<EditingEnvironmentalManagementPlan | null>(null);
  ...
  const fetchPlans = useCallback(async () => {
    try {
      ...
      if (isConnected) {
        const result = await listEnvironmentalManagementPlanByCorpo({
          corpo_id: corpoId,
          refreshAccessToken,
          logout,
        });
        ...
        if (result.status && result.data) {
          setPlans(result.data as EnvironmentalManagementPlan[]);
        } else {
          setPlans([]);
        }
      } else {
        ...
        if (cacheStr) {
          ...
          const plansCache = cache.filter((item: any) => item.type === 'environmental_management_plan');
          setPlans(plansCache);
        } else {
          setPlans([]);
        }
      ...
      setError('Error al cargar los planes de gestión ambiental');
      try {
        const cacheStr = await AsyncStorage.getItem('evaluations_cache');
        if (cacheStr) {
          const cache = JSON.parse(cacheStr);
  ...
  const startEditing = (record: EnvironmentalManagementPlan) => {
    setIsCreating(false);
    const parsedSecciones: { [key: string]: SeccionElement[] } = {};
    SECCIONES_PRINCIPALES.forEach(sec => {
      parsedSecciones[sec.key] = parseSeccion(record[sec.key as keyof EnvironmentalManagementPlan] as string | null);
    });
    ...
    setEditingRecord({
      ...
      metodologia: parsedSecciones.metodologia,
      rol_horario: parsedSecciones.rol_horario,
      uniformes: parsedSecciones.uniformes,
      equipos: parsedSecciones.equipos,
      accesorios_varios: parsedSecciones.accesorios_varios,
      tiempo_respuesta: parsedSecciones.tiempo_respuesta,
      distribucion_labores: parsedSecciones.distribucion_labores,
      frecuencia_limpieza: parsedSecciones.frecuencia_limpieza,
      supervision: parsedSecciones.supervision,
      estrategia: parsedSecciones.estrategia,
      responsable: parsedSecciones.responsable,
      formularios: parsedSecciones.formularios,
  ...
  const savePlanHandler = async () => {
    ...
    Alert.alert(
      'Confirmar',
      '¿Estás seguro de que deseas guardar este plan de gestión ambiental?',
      [
          ...
          onPress: async () => {
            try {
              const requestData = {
                ...
                metodologia: secciones.metodologia.length > 0 ? JSON.stringify(secciones.metodologia) : null,
                rol_horario: secciones.rol_horario.length > 0 ? JSON.stringify(secciones.rol_horario) : null,
                ...
                tiempo_respuesta: secciones.tiempo_respuesta.length > 0 ? JSON.stringify(secciones.tiempo_respuesta) : null,
                distribucion_labores: secciones.distribucion_labores.length > 0 ? JSON.stringify(secciones.distribucion_labores) : null,
                frecuencia_limpieza: secciones.frecuencia_limpieza.length > 0 ? JSON.stringify(secciones.frecuencia_limpieza) : null,
                supervision: secciones.supervision.length > 0 ? JSON.stringify(secciones.supervision) : null,
                estrategia: secciones.estrategia.length > 0 ? JSON.stringify(secciones.estrategia) : null,
                responsable: secciones.responsable.length > 0 ? JSON.stringify(secciones.responsable) : null,
                formularios: secciones.formularios.length > 0 ? JSON.stringify(secciones.formularios) : null,
              ...
              if (isConnected) {
                ...
                if (result.status) {
                  Alert.alert('Éxito', 'Plan de gestión ambiental guardado correctamente');
                  cancelCreating();
                  fetchPlans();
                } else {
                  Alert.alert('Error', result.message || 'Error al guardar el plan de gestión ambiental');
                }
              } else {
                ...
                actions.push({
                  id: localId,
                  action: 'create',
                  type: 'environmental_management_plan',
                  payload: requestData,
                  synced: false,
                });
                ...
                const newRecordCache: EnvironmentalManagementPlan = {
                  id: '',
                  id_local: localId,
                  ubicacion: ubicacion.trim() || null,
                  objetivo: secciones.objetivo.length > 0 ? JSON.stringify(secciones.objetivo) : null,
                ...
                cache.push({ ...newRecordCache, type: 'environmental_management_plan' });
                await AsyncStorage.setItem('evaluations_cache', JSON.stringify(cache));

                Alert.alert('Modo Offline', 'Plan de gestión ambiental registrado localmente. Se sincronizará cuando haya conexión.');
                cancelCreating();
                fetchPlans();
              }
            } catch (err) {
              console.error('Error saving plan:', err);
              Alert.alert('Error', 'No se pudo guardar el plan de gestión ambiental');
            }
          },
        },
      ]
  ...
  const updatePlanHandler = async () => {
    ...
    Alert.alert(
      'Confirmar',
      '¿Estás seguro de que deseas actualizar este plan de gestión ambiental?',
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          text: 'Confirmar',
          onPress: async () => {
            try {
              ...
              if (isConnected) {
                ...
                if (result.status) {
                  Alert.alert('Éxito', 'Plan de gestión ambiental actualizado correctamente');
                  cancelEditing();
                  fetchPlans();
                } else {
                  Alert.alert('Error', result.message || 'Error al actualizar el plan de gestión ambiental');
                }
              } else {
                const actionsStr = await AsyncStorage.getItem('evaluations_actions');
                const actions = actionsStr ? JSON.parse(actionsStr) : [];
                ...
                if (cacheStr) {
                  ...
                  const updatedCache = cache.map((item: any) => {
                    if ((item.id === recordId || item.id_local === recordId) && item.type === 'environmental_management_plan') {
                      return {
                        ...item,
                        ubicacion: requestData.ubicacion,
                        objetivo: requestData.objetivo,
                        metodologia: requestData.metodologia,
                        rol_horario: requestData.rol_horario,
                        uniformes: requestData.uniformes,
                        equipos: requestData.equipos,
                        accesorios_varios: requestData.accesorios_varios,
                        tiempo_respuesta: requestData.tiempo_respuesta,
                        distribucion_labores: requestData.distribucion_labores,
                        frecuencia_limpieza: requestData.frecuencia_limpieza,
                        supervision: requestData.supervision,
                        estrategia: requestData.estrategia,
                        responsable: requestData.responsable,
                        formularios: requestData.formularios,
                ...
                Alert.alert('Modo Offline', 'Plan de gestión ambiental actualizado localmente. Se sincronizará cuando haya conexión.');
                cancelEditing();
                fetchPlans();
              }
            } catch (err) {
              console.error('Error updating plan:', err);
              Alert.alert('Error', 'No se pudo actualizar el plan de gestión ambiental');
            }
          },
        },
      ]
  ...
  const deletePlanHandler = async (record: EnvironmentalManagementPlan) => {
    ...
    Alert.alert(
      'Confirmar',
      '¿Estás seguro de que deseas eliminar este plan de gestión ambiental?',
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          text: 'Confirmar',
          onPress: async () => {
            try {
              ...
              if (isConnected) {
                ...
                if (result.status) {
                  Alert.alert('Éxito', 'Plan de gestión ambiental eliminado correctamente');
                  fetchPlans();
                } else {
                  Alert.alert('Error', result.message || 'Error al eliminar el plan de gestión ambiental');
                }
              } else {
                const actionsStr = await AsyncStorage.getItem('evaluations_actions');
                const actions = actionsStr ? JSON.parse(actionsStr) : [];
                ...
                Alert.alert('Modo Offline', 'Plan de gestión ambiental marcado para eliminación localmente. Se sincronizará cuando haya conexión.');
                fetchPlans();
              }
            } catch (err) {
              console.error('Error deleting plan:', err);
              Alert.alert('Error', 'No se pudo eliminar el plan de gestión ambiental');
            }
          },
        },
      ]
  ...
  const getActionIcon = (action: string) => {
    switch (action.toLowerCase()) {
      case 'plan': return <Ionicons name="leaf" size={24} color='#000000' />;
      ...
      default: return <Ionicons name="leaf" size={24} color='#000000' />;
    }
  };
    ...
    if (isLoading) {
      return (
        <ThemedView style={styles.loadingContainer}>
          ...
          <ThemedText style={styles.loadingText}>Cargando planes de gestión ambiental...</ThemedText>
        </ThemedView>
      );
    }
    ...
    if (plans.length === 0) {
      return (
        <ThemedView style={styles.emptyContainer}>
          <ThemedText style={styles.emptyText}>No hay planes de gestión ambiental registrados.</ThemedText>
        </ThemedView>
      );
    }
  ...
  return (
    <ThemedView style={styles.container}>
      <AppHeader onMenuPress={handleMenuPress} title="Plan de Gestión Ambiental" />
      ...
      >
        <ThemedView style={styles.contentContainer}>
          <ThemedText type="title" style={styles.screenTitle}>
            {getActionIcon('plan')} Plan de Gestión Ambiental
          </ThemedText>
          ...
          {isCreating || editingRecord ? (
            <ThemedView style={styles.formContainer}>
              ...
              <ThemedView style={styles.formGroup}>
                <ThemedText style={styles.formLabel}>Ubicación</ThemedText>
                <TextInput
            ...
            <ThemedView style={styles.listSection}>
              <TouchableOpacity style={styles.createButton} onPress={startCreating}>
                ...
                <ThemedText style={styles.createButtonText}>Crear Nuevo Plan de Gestión Ambiental</ThemedText>
              </TouchableOpacity>
              {renderPlanList()}
            </ThemedView>
          )}
      ...
      <SlideMenu
        ...
        currentRoute="EnvironmentalManagementPlan"
      />
    </ThemedView>
  );
}
