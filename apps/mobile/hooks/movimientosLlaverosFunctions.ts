...
export type MovimientoLlaveroItem = {
  id: number;
  llavero_id: number;
...
export async function listMovimientosLlavero({
  llaveroId,
  marcaId,
  refreshAccessToken,
  logout,
}: ListParams): Promise<{ status: boolean; data?: MovimientoLlaveroItem[]; message?: string }> {
  try {
    const apiUrl = getApiUrl();
    const { refreshAccessToken: refresh, logout: doLogout } = requireAuthHandlers(refreshAccessToken, logout);

    const response = await authedFetch({
      url: `${apiUrl}/api/llaveros/${llaveroId}/movimientos?m=${marcaId}`,
      init: {
        method: 'GET',
        headers: {
    ...
    console.error('Error listing movimientos llavero:', error);
    return { status: false, message: error.message || 'Error al cargar movimientos' };
  }
}

export async function createMovimientoLlavero({
  llaveroId,
  requestData,
  refreshAccessToken,
  logout,
  ...
  try {
    ...
    const response = await authedFetch({
      url: `${apiUrl}/api/llaveros/${llaveroId}/movimientos`,
      init: {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
    ...
    console.error('Error creating movimiento llavero:', error);
    return { status: false, message: error.message || 'Error al crear movimiento' };
  }
}

export async function updateMovimientoLlavero({
  llaveroId,
  id,
  requestData,
  refreshAccessToken,
  ...
  try {
    ...
    const response = await authedFetch({
      url: `${apiUrl}/api/llaveros/${llaveroId}/movimientos/${id}`,
      init: {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
    ...
    console.error('Error updating movimiento llavero:', error);
    return { status: false, message: error.message || 'Error al actualizar movimiento' };
  }
}

export async function deleteMovimientoLlavero({
  llaveroId,
  id,
  marcaId,
  refreshAccessToken,
  ...
  try {
    ...
    const response = await authedFetch({
      url: `${apiUrl}/api/llaveros/${llaveroId}/movimientos/${id}?m=${marcaId}`,
      init: {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
    ...
    console.error('Error deleting movimiento llavero:', error);
