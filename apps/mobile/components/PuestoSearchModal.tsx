...
import { searchPuestosForTraining, type PuestoSearchHit } from '@/hooks/puestoSearch';

export type { PuestoSearchHit };

export interface PuestoSearchModalProps {
  visible: boolean;
  structure?: any[];
  onClose: () => void;
  onSelect: (hit: PuestoSearchHit) => void;
}

export default function PuestoSearchModal({
  visible,
  structure,
  onClose,
  onSelect,
}: PuestoSearchModalProps) {
  const [query, setQuery] = useState('');
  const [isSearching, setIsSearching] = useState(false);
  const [hits, setHits] = useState<PuestoSearchHit[]>([]);
  ...
  const runSearch = useCallback(async () => {
    ...
    if (!trimmed) {
      setSearchError('Escriba un nombre o código de puesto para buscar.');
      setHits([]);
      setHasSearched(false);
      return;
    }
    ...
    try {
      const found = await searchPuestosForTraining(trimmed, structure);
      setHits(found);
      if (found.length === 0) {
        setSearchError('No se encontraron puestos en la jerarquía local.');
      }
    } catch {
      setHits([]);
  ...
  const handlePick = (hit: PuestoSearchHit) => {
    onSelect(hit);
    onClose();
  };
      ...
      <View style={styles.backdrop}>
        <ThemedView style={styles.card}>
          <View style={styles.header}>
            <ThemedText style={styles.title}>Buscar puesto</ThemedText>
          ...
          >
            ...
            <View style={styles.searchRow}>
              <TextInput
                ...
                placeholder="Nombre o código del puesto"
