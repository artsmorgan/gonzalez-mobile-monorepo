...
export type LlavesModuleFilters = ActaEntregaModuleFilters & {
    entregadoPorContains?: string | null;
    recibidoPorContains?: string | null;
    llaveroIds?: number[] | null;
};

export type LlavesOrderKey = "empresa_id" | "cliente_id" | "division_id" | "contrato_id" | "corpo_id" | "puesto_id" | "created_at";
...
export function normalizeLlavesFilters(raw: unknown): LlavesModuleFilters {
    ...
    return {
        ...
        llaveroIds: parseIds(
            Array.isArray(o.llaveroIds)
                ? o.llaveroIds
                : typeof o.llaveroIds === "string"
                  ? String(o.llaveroIds)
                        .split(",")
                        .map((x) => Number(x.trim()))
                  : [],
...
export function hasLlavesListModuleFiltersContent(f: LlavesModuleFilters): boolean {
    ...
    if (f.llaveroIds?.length) return true;
    if (f.entregadoPorContains || f.recibidoPorContains) return true;
    return false;
}

export function filtersMatchLlavesListQuery(parsedRowFilters: any, listModuleFilters?: LlavesModuleFilters): boolean {
    if (!listModuleFilters) return true;
    const saved = normalizeLlavesFilters((parsedRowFilters?.moduleFilters || {}) as any);
    ...
    if (!overlaps(listModuleFilters.llaveroIds ?? undefined, saved.llaveroIds ?? undefined)) return false;
...
export async function queryLlavesRows(prisma: ReportDataAccess, filters: LlavesModuleFilters, orderKey: LlavesOrderKey) {
    ...
    if (filters.llaveroIds?.length) {
        where.e_llave_en_llavero = { some: { llavero_id: { in: filters.llaveroIds } } };
    }
    ...
    if (Object.keys(movimientoWhere).length > 0) {
        where.e_movimiento_llave = { some: movimientoWhere };
    }
    ...
    const rows = await prisma.e_llave.findMany({
        ...
        include: {
            ...
            e_llave_en_llavero: {
                include: {
                    e_llavero: { select: { id: true, nombre_llavero: true } },
                },
            },
        },
        orderBy: { id: "desc" },
    ...
    const enriched = rows.map((r: any) => {
        ...
        return {
            ...
            movimientos_count: Array.isArray(r.e_movimiento_llave) ? r.e_movimiento_llave.length : 0,
        };
    });
...
export async function buildLlavesExcelConsolidado(rows: any[]): Promise<Buffer> { // Consolidado
    const wb = new ExcelJS.Workbook();
    const wsMain = wb.addWorksheet("Llaves");
    ...
    for (const r of [...rows].sort((a, b) => Number(b.id) - Number(a.id))) {
        const start = wsMov.rowCount + 1;
        movAnchorByKey.set(Number(r.id), start);
        wsMov.mergeCells(start, 1, start, 11);
        wsMov.getCell(start, 1).value = `Llave #${r.id} | ${r.numero_llave || ""} | ${r.empresa_nombre || ""}`;
        wsMov.getCell(start, 1).font = { bold: true };
        wsMov.getCell(start, 1).fill = hdrFill;
        for (let c = 1; c <= 11; c++) wsMov.getCell(start, c).border = border;
...
async function buildLlavesIndividualWorkbook(rows: any[], reportName: string): Promise<Buffer> {
    const wb = new ExcelJS.Workbook();
    const ws = wb.addWorksheet("Registro llaves");
    const border: Partial<ExcelJS.Borders> = { top: { style: "thin" }, left: { style: "thin" }, bottom: { style: "thin" }, right: { style: "thin" } };
    const hdrFill = { type: "pattern", pattern: "solid", fgColor: { argb: "FF102D4E" } } as const;

    ws.columns = [4.5, 18, 24, 24, 24].map((w) => ({ width: w }));
    ...
    ws.mergeCells("A1:B1");
    ws.mergeCells("C1:D1");
    ws.getCell("C1").value = "REGISTRO DE LLAVES";
    ...
    // Sin borde inferior en A4: evita línea superior visual en A5
    ws.getCell("A4").border = { left: { style: "thin" } };

    ws.getCell("B5").value = "# Llave";
    ws.getCell("C5").value = "Lugar que abre";
    ws.getCell("D5").value = "Cantidad de copias";
    ...
    for (let i = 0; i < rows.length; i += 1) {
        ...
        rr.getCell(2).value = excelCellString(r.numero_llave);
        rr.getCell(3).value = excelCellString(r.lugar_abre);
        rr.getCell(4).value = String(r.cantidad_copias ?? "");
    ...
    for (const r of rows) {
        const movWs = wb.addWorksheet(`Movimiento llaves ${String(r.id)}`.slice(0, 31));
        ...
        movWs.getCell("C1").value = "MOVIMIENTO DE LLAVES";
        ...
        for (let r = 2; r <= 3; r += 1) {
            for (let c = 1; c <= 10; c += 1) {
                movWs.getCell(r, c).fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FFFFFFFF" } };
            }
        }
        ...
        const headers = [
            ...
            "Nombre de la persona que entrega o recibe las llaves",
...
export async function buildLlavesIndividualZip(rows: any[], reportName: string): Promise<Buffer> {
    ...
    for (const [corpoId, list] of grouped.entries()) {
        const wb = await buildLlavesIndividualWorkbook(list, reportName);
        archive.append(wb, { name: `Llaves_Corpo_${corpoId}.xlsx` });
    }
