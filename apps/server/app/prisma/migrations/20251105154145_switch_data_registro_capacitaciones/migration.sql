/*
  ...
  - Added the required column `responsable_id` to the `e_registro_capacitaciones` table without a default value. This is not possible if the table is not empty.

*/
-- DropForeignKey
ALTER TABLE `e_registro_capacitaciones` DROP FOREIGN KEY `e_registro_capacitaciones_empleado_id_fkey`;

-- DropIndex
DROP INDEX `e_registro_capacitaciones_empleado_id_fkey` ON `e_registro_capacitaciones`;

-- AlterTable
ALTER TABLE `c_evaluacion_empleado` MODIFY `created_at` DATETIME NOT NULL;

-- AlterTable
ALTER TABLE `e_registro_capacitaciones` ADD COLUMN `responsable_id` INTEGER NOT NULL,
    MODIFY `cedula_empleado` VARCHAR(25) NOT NULL,
    MODIFY `cedula_responsable` VARCHAR(25) NOT NULL;
