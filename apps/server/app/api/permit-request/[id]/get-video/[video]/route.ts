...
export async function GET(
  ...
  try {
    ...
    const videoNameParam = resolvedParams.video;
    if (!idNum || !videoNameParam) {
      await reportError(req, "api/permit-request/[id]/get-video/[video]", "GET", 400, "ID o video faltante");
    ...
    if (!currentEmployeeId) {
      await reportError(req, "api/permit-request/[id]/get-video/[video]", "GET", 400, "Empleado inválido");
    ...
    if (!record) {
      await reportError(req, "api/permit-request/[id]/get-video/[video]", "GET", 404, "Solicitud no encontrada");
      return NextResponse.json({ status: false, message: "Solicitud no encontrada" }, { status: 404 });
    }
    if ((record as any).isActive === false) {
      await reportError(req, "api/permit-request/[id]/get-video/[video]", "GET", 404, "Solicitud no disponible");
    ...
    if (!fileRecord) {
      await reportError(req, "api/permit-request/[id]/get-video/[video]", "GET", 404, "Video no encontrado");
    ...
    if (!isExecutive) {
      await reportError(req, "api/permit-request/[id]/get-video/[video]", "GET", 400, "Solo el ejecutivo asignado puede ver este archivo");
    ...
    await reportError(req, "api/permit-request/[id]/get-video/[video]", "GET", 500, errorMessage);
