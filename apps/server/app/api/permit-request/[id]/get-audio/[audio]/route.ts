...
export async function GET(
  ...
  try {
    ...
    const audioNameParam = resolvedParams.audio;
    if (!idNum || !audioNameParam) {
      await reportError(req, "api/permit-request/[id]/get-audio/[audio]", "GET", 400, "ID o audio faltante");
      return NextResponse.json({ status: false, message: "ID o audio faltante" }, { status: 400 });
    }
    ...
    if (!currentEmployeeId) {
      await reportError(req, "api/permit-request/[id]/get-audio/[audio]", "GET", 400, "Empleado inválido");
    ...
    if (!record) {
      await reportError(req, "api/permit-request/[id]/get-audio/[audio]", "GET", 404, "Solicitud no encontrada");
      return NextResponse.json({ status: false, message: "Solicitud no encontrada" }, { status: 404 });
    }
    if ((record as any).isActive === false) {
      await reportError(req, "api/permit-request/[id]/get-audio/[audio]", "GET", 404, "Solicitud no disponible");
      return NextResponse.json({ status: false, message: "Solicitud no disponible" }, { status: 404 });
    }

    const decodedFileName = decodeURIComponent(String(audioNameParam).trim());
    ...
    if (!fileRecord) {
      await reportError(req, "api/permit-request/[id]/get-audio/[audio]", "GET", 404, "Audio no encontrado");
    ...
    if (!isExecutive) {
      await reportError(req, "api/permit-request/[id]/get-audio/[audio]", "GET", 400, "Solo el ejecutivo asignado puede ver este archivo");
    ...
    await reportError(req, "api/permit-request/[id]/get-audio/[audio]", "GET", 500, errorMessage);
