...
export async function GET(req: NextRequest, context: { params: Promise<{ id: string }> }) {
    try {
        ...
        if (!llaveroId) {
            await reportError(req, "api/llaveros/[id]/movimientos", "GET", 400, "ID no especificado");
            return NextResponse.json({ status: false, message: "ID no especificado" }, { status: 400 });
        }

        const marcaIdStr = req.nextUrl.searchParams.get("m");
        if (!marcaIdStr) {
            await reportError(req, "api/llaveros/[id]/movimientos", "GET", 400, "Marca no especificada");
        ...
        if (!own.ok) {
            await reportError(req, "api/llaveros/[id]/movimientos", "GET", 404, own.message);
        ...
        await reportError(req, "api/llaveros/[id]/movimientos", "GET", 500, errorMessage);
...
export async function POST(req: NextRequest, context: { params: Promise<{ id: string }> }) {
    try {
        ...
        const llaveroId = parseInt(resolvedParams.id);
        if (!llaveroId) {
            await reportError(req, "api/llaveros/[id]/movimientos", "POST", 400, "ID no especificado");
            return NextResponse.json({ status: false, message: "ID no especificado" }, { status: 400 });
        }
        ...
        if (!llavero) {
            await reportError(req, "api/llaveros/[id]/movimientos", "POST", 404, "Llavero no encontrado");
            return NextResponse.json({ status: false, message: "Llavero no encontrado" }, { status: 404 });
        }
        ...
        if (!marca_id) {
            await reportError(req, "api/llaveros/[id]/movimientos", "POST", 400, "Marca no especificada");
            return NextResponse.json({ status: false, message: "Marca no especificada" }, { status: 400 });
        }

        const own = await validateLlaveroOwnership(req, llaveroId, parseInt(String(marca_id)));
        if (!own.ok) {
            await reportError(req, "api/llaveros/[id]/movimientos", "POST", 404, own.message);
            return NextResponse.json({ status: false, message: own.message }, { status: 404 });
        }
        ...
        if (!fechaDate || !horaNormRes.ok) {
            await reportError(req, "api/llaveros/[id]/movimientos", "POST", 400, "Fecha u hora inválida");
        ...
        if (requiredStrings.some((v) => typeof v !== "string" || v.trim().length === 0)) {
            await reportError(req, "api/llaveros/[id]/movimientos", "POST", 400, "Datos incompletos");
        ...
        if (created) {
            ...
            if (llavero.corpo_id) {
                const sucursal = await prisma.e_estructura_sucursal.findUnique({ where: { id: llavero.corpo_id } });
                if (sucursal) {
                    sucursalNombre = sucursal.nombre;
                }
            }

            const description = "Se ha registrado un movimiento del llavero " + llavero.nombre_llavero + " - " + llavero.numero_llavero + " de la sucursal " + sucursalNombre + " el día " + fechaRegistro + " a las " + horaRegistro + " (Del empleado " + nombre_persona_entrega + " a " + nombre_persona_recibe + ")";
        ...
        await reportError(req, "api/llaveros/[id]/movimientos", "POST", 500, errorMessage);
        return NextResponse.json({ status: false, message: errorMessage }, { status: 500 });
    }
}
