...
export async function GET(req: NextRequest, context: { params: Promise<{ id: string; audio: string }> }) {
  ...
  if (!id || !audio) {
    await reportError(req, "api/articulo-mantenimiento/[id]/get-audio/[audio]", "GET", 400, "ID o audio faltante");
  ...
  try {
    ...
    if (!mantenimiento) {
      await reportError(req, "api/articulo-mantenimiento/[id]/get-audio/[audio]", "GET", 404, "Mantenimiento no encontrado");
    ...
    if (!fileRecord) {
      await reportError(req, "api/articulo-mantenimiento/[id]/get-audio/[audio]", "GET", 404, "Archivo no encontrado");
    ...
    await reportError(req, "api/articulo-mantenimiento/[id]/get-audio/[audio]", "GET", 400, errorMessage);
