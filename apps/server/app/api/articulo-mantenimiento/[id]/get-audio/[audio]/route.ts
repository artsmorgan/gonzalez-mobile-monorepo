...
export async function GET(req: NextRequest, context: { params: Promise<{ id: string; audio: string }> }) {
  ...
  const fileRecord = await prisma.c_archivos_adjuntos_articulo_mantenimiento.findFirst({
    where: { activo_mantenimiento_id: mantenimiento.id, name: audio },
  });
  if (!fileRecord) return NextResponse.json({ status: false, message: "Archivo no encontrado" }, { status: 404 });

  const filePath = path.join(process.cwd(), "public", "uploads", "articulo-mantenimiento", `${mantenimiento.id}`, audio);
  if (!fs.existsSync(filePath)) return NextResponse.json({ status: false, message: "Audio no encontrado" }, { status: 404 });
