...
export async function GET(req: NextRequest, context: { params: Promise<{ id: string; image: string }> }) {
  ...
  if (!id || !image) {
    await reportError(req, "api/articulo-mantenimiento/[id]/get-image/[image]", "GET", 400, "ID o imagen faltante");
  ...
  try {
    ...
    if (!mantenimiento) {
      await reportError(req, "api/articulo-mantenimiento/[id]/get-image/[image]", "GET", 404, "Mantenimiento no encontrado");
    ...
    if (!fileRecord) {
      await reportError(req, "api/articulo-mantenimiento/[id]/get-image/[image]", "GET", 404, "Archivo no encontrado");
    ...
    await reportError(req, "api/articulo-mantenimiento/[id]/get-image/[image]", "GET", 400, errorMessage);
