...
export async function GET(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!asignadoId) {
      await reportError(req, "api/articulo-mantenimiento/asignado/[id]/movimientos", "GET", 400, "ID no especificado");
      return NextResponse.json({ status: false, message: "ID no especificado" }, { status: 400 });
    }

    const marcaIdStr = req.nextUrl.searchParams.get("m");
    if (!marcaIdStr) {
      await reportError(req, "api/articulo-mantenimiento/asignado/[id]/movimientos", "GET", 400, "Marca no especificada");
    ...
    if (!own.ok) {
      await reportError(req, "api/articulo-mantenimiento/asignado/[id]/movimientos", "GET", 400, own.message);
    ...
    await reportError(req, "api/articulo-mantenimiento/asignado/[id]/movimientos", "GET", 500, errorMessage);
...
export async function POST(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const asignadoId = parseInt(resolvedParams.id);
    if (!asignadoId) {
      await reportError(req, "api/articulo-mantenimiento/asignado/[id]/movimientos", "POST", 400, "ID no especificado");
      return NextResponse.json({ status: false, message: "ID no especificado" }, { status: 400 });
    }
    ...
    if (!marca_id) {
      await reportError(req, "api/articulo-mantenimiento/asignado/[id]/movimientos", "POST", 400, "Marca no especificada");
      return NextResponse.json({ status: false, message: "Marca no especificada" }, { status: 400 });
    }

    const own = await validateAsignadoOwnership(req, asignadoId, parseInt(String(marca_id)));
    if (!own.ok) {
      await reportError(req, "api/articulo-mantenimiento/asignado/[id]/movimientos", "POST", 400, own.message);
      return NextResponse.json({ status: false, message: own.message }, { status: 400 });
    }
    ...
    if (!fechaDate || !horaDate) {
      await reportError(req, "api/articulo-mantenimiento/asignado/[id]/movimientos", "POST", 400, "Fecha u hora inválida");
    ...
    if (requiredStrings.some((v) => typeof v !== "string" || v.trim().length === 0)) {
      await reportError(req, "api/articulo-mantenimiento/asignado/[id]/movimientos", "POST", 400, "Datos incompletos");
    ...
    await reportError(req, "api/articulo-mantenimiento/asignado/[id]/movimientos", "POST", 500, errorMessage);
    return NextResponse.json({ status: false, message: errorMessage }, { status: 500 });
  }
}
