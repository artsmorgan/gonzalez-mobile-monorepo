...
export async function POST(req: NextRequest) {
    try {
        ...
        const new_record = await prisma.c_plan_comunicacion.create({
            data: {
                ...
                responsable_aprobacion: responsable_aprobacion || null,
                puesto_aprobacion: puesto_aprobacion || null,
        ...
        return NextResponse.json({ 
            status: true, 
            message: "Plan de comunicación creado correctamente",
            data: new_record
        }, { status: 200 });
