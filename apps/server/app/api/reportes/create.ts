...
import { createReportPrismaClient, type ReportDataAccess } from "../../../utils/reportDynamicPrisma";
import { resolveReportMobileRelativeUrl } from "../../../utils/reportesMobileFile";
import { resolveUserAccessToken } from "../../../utils/resolveUserAccessToken";
...
>(reportDb: ReportDataAccess, rows: T[]) {
    const creatorIds = [
...
export async function executeReportesOperation(
    req: NextRequest,
    payload: ReportesPayload,
): Promise<ReportesOperationResult> {
    try {
        ...
        const reportDb = createReportPrismaClient(req, payload.token);
        ...
        if (op === "searchEmployees") {
            ...
            const rows = await reportDb.c_empleado.findMany({
                where: {
                    OR: [
                        { codigo: { contains: q } },
                        { cedula: { contains: q } },
        ...
        if (op === "searchActaStructure") {
            ...
            const q = String(payload.q || "").trim();
            if (!entity) {
                return { status: false, message: "entity es obligatorio" };
            }
            if (q.length < 1) {
                return { status: true, data: [] };
            }
            ...
            if (entity === "empresa") {
                const rows = await reportDb.e_estructura_empresa.findMany({
                    where: {
                        deleted: null,
            ...
            if (entity === "cliente") {
                ...
                const rows = await reportDb.e_estructura_cliente.findMany({
            ...
            if (entity === "division") {
                ...
                const rows = await reportDb.n_division.findMany({
                    where: {
                        OR: [{ nombre: { contains: q } }, { codigo: { contains: q } }],
                    },
                    take: 50,
            ...
            if (entity === "contrato") {
                const rows = await reportDb.e_estructura_contrato.findMany({
                    where: {
                        deleted: null,
                        AND: [
            ...
            if (entity === "corpo") {
                const rows = await reportDb.e_estructura_sucursal.findMany({
                    where: {
                        deleted: null,
                        AND: [
                            { OR: activeOrNoInactiveDate },
            ...
            if (entity === "puesto") {
                const rows = await reportDb.e_estructura_puesto.findMany({
                    where: {
                        deleted: null,
                        AND: [
                            { OR: activeOrNoInactiveDate },
            ...
            if (entity === "plaza") {
                const rows = await reportDb.e_estructura_plazas.findMany({
                    where: {
                        deleted: null,
                        AND: [
                            { OR: activeOrNoInactiveDate },
            ...
            if (entity === "llavero") {
                const rows = await reportDb.e_llavero.findMany({
                    where: {
                        isActive: true,
            ...
            if (entity === "ejecutivo") {
                const rows = await reportDb.n_ejecutivo_cuenta.findMany({
            ...
            return { status: false, message: `entity no soportada: ${entity}` };
        }
        ...
        if (op === "searchAlmuerzoCedulas") {
            ...
            const rows = await reportDb.c_empleado_almuerzo.findMany({
                where: {
                    isActive: true,
        ...
        if (op === "previewUserLoginRefreshTokens") {
            ...
            const rows = await queryRefreshTokensUserLogin(reportDb, mf, orderKey);
            ...
            return { status: true, data: slim, count: slim.length };
        }
        ...
        if (op === "previewActaEntregaProductos") {
            ...
            const rows = await queryActaEntregaProductos(reportDb, mf, orderKey);
            return { status: true, data: rows, count: rows.length };
        }
        ...
        if (op === "previewEntregaPuesto") {
            ...
            const rows = await queryEntregaPuestoRows(reportDb, mf, orderKey);
            return { status: true, data: rows, count: rows.length };
        }
        ...
        if (op === "previewAgendaMinuta") {
            ...
            const rows = await queryAgendaMinutaReportRows(reportDb, mf, orderKey);
            return { status: true, data: rows, count: rows.length };
        }
        ...
        if (op === "previewAperturaCierrePuesto") {
            ...
            const rows = await queryAperturaCierrePuestoRows(reportDb, mf, orderKey);
            return { status: true, data: rows, count: rows.length };
        }
        ...
        if (op === "previewVulnerabilidad") {
            ...
            const rows = await queryVulnerabilidadRows(reportDb, mf, orderKey);
            return { status: true, data: rows, count: rows.length };
        }
        ...
        if (op === "previewActividades") {
            ...
            const rows = await queryActividadesReportRows(reportDb, mf, orderKey);
            return { status: true, data: rows, count: rows.length };
        }
        if (op === "previewControlAsistencia") {
            ...
            const rows = await queryControlAsistenciaRows(reportDb, mf, orderKey);
            return { status: true, data: rows, count: rows.length };
        }
        if (op === "previewDocumentosEntregados") {
            ...
            const rows = await queryDocumentosEntregadosRows(reportDb, mf, orderKey);
            return { status: true, data: rows, count: rows.length };
        }
        if (op === "previewEncuestaSatisfaccion") {
            ...
            const rows = await queryEncuestaSatisfaccionRows(reportDb, mf, orderKey);
            return { status: true, data: rows, count: rows.length };
        }
        if (op === "previewMutuosAcuerdos") {
            ...
            const rows = await queryMutuosAcuerdosRows(reportDb, mf, orderKey);
            return { status: true, data: rows, count: rows.length };
        }
        ...
        if (op === "previewAccionesPersonales") {
            ...
            const rows = await queryAccionesPersonalesRows(reportDb, mf, orderKey);
            return { status: true, data: rows, count: rows.length };
        }
        if (op === "previewIncidentes") {
            ...
            const rows = await queryIncidenteRows(reportDb, mf, orderKey);
            return { status: true, data: rows, count: rows.length };
        }
        if (op === "previewLlaves") {
            ...
            const rows = await queryLlavesRows(reportDb, mf, orderKey);
            return { status: true, data: rows, count: rows.length };
        }
        if (op === "previewLlaveros") {
            ...
            const rows = await queryLlaverosRows(reportDb, mf, orderKey);
            return { status: true, data: rows, count: rows.length };
        }
        if (op === "previewBitacoraNovedades") {
            ...
            const rows = await queryBitacoraNovedadesRows(reportDb, mf, orderKey);
            return { status: true, data: rows, count: rows.length };
        }
        if (op === "previewMaestroQuejas") {
            ...
            const rows = await queryMaestroQuejasRows(reportDb, mf, orderKey);
            return { status: true, data: rows, count: rows.length };
        }
        ...
        if (op === "previewChecklistSupervision") {
            ...
            const rows = await queryChecklistSupervisionRows(reportDb, mf, orderKey);
            return { status: true, data: rows, count: rows.length };
        }
        ...
        if (op === "previewEvaluacionPersonal") {
            ...
            const rows = await queryEvaluacionEmpleadoRows(reportDb, mf, orderKey);
            return { status: true, data: rows, count: rows.length };
        }
        ...
        if (op === "previewProductoNoConforme") {
            ...
            const rows = await queryProductoNoConformeRows(reportDb, mf, orderKey);
            return { status: true, data: rows, count: rows.length };
        }
        ...
        if (op === "previewInduccionRecorrido") {
            ...
            const rows = await queryInduccionRecorridoRows(reportDb, mf, orderKey);
            return { status: true, data: rows, count: rows.length };
        }
        ...
        if (op === "previewManualesPuesto") {
            ...
            const rows = await queryManualesPuestoRows(reportDb, mf, orderKey);
            return { status: true, data: rows, count: rows.length };
        }
        ...
        if (op === "previewArticulosPuesto") {
            ...
            const rows = await queryArticulosPuestoRows(reportDb, mf, orderKey);
            return { status: true, data: rows.slice(0, 100), count: rows.length };
        }
        ...
        if (op === "previewMantenimientoArticulos") {
            ...
            const rows = await queryMantenimientoArticulosRows(reportDb, mf, orderKey);
            return { status: true, data: rows.slice(0, 100), count: rows.length };
        }
        ...
        if (op === "previewRegistroVehiculosCorporativos") {
            ...
            const rows = await queryRegistroVehiculosCorporativosRows(reportDb, mf, orderKey);
            return { status: true, data: rows.slice(0, 100), count: rows.length };
        }
        ...
        if (op === "previewRevisionVehiculos") {
            ...
            const rows = await queryRevisionVehiculosRows(reportDb, mf, orderKey);
            return { status: true, data: rows.slice(0, 100), count: rows.length };
        }
        ...
        if (op === "searchCorporateVehicles") {
            ...
            const rows = await searchCorporateVehiclesForReport(reportDb, q);
            return { status: true, data: rows };
        }
        ...
        if (op === "previewRegistroVisitas") {
            ...
            const rows = await queryRegistroVisitasRows(reportDb, mf, orderKey, { take: 100 });
            return { status: true, data: rows, count: rows.length };
        }
        ...
        if (op === "previewVisitasVehiculos") {
            ...
            const rows = await queryVisitasVehiculosRows(reportDb, mf, orderKey, { take: 100 });
            return { status: true, data: rows, count: rows.length };
        }
        if (op === "previewNotasVoz") {
            ...
            const rows = await queryNotasVozRows(reportDb, mf, orderKey);
            return { status: true, data: rows.slice(0, 100), count: rows.length };
        }
        if (op === "previewCambiosUbicacionPuesto") {
            ...
            const rows = await queryCambiosUbicacionPuestoRows(reportDb, mf, orderKey);
            return { status: true, data: rows.slice(0, 100), count: rows.length };
        }
        if (op === "previewRegistroCapacitaciones") {
            ...
            const rows = await queryRegistroCapacitacionesRows(reportDb, mf, orderKey);
            return { status: true, data: rows.slice(0, 100), count: rows.length };
        }
        ...
        if (op === "previewRegistroInduccionGeneral") {
            ...
            const rows = await queryRegistroInduccionGeneralRows(reportDb, mf, orderKey);
            return { status: true, data: rows.slice(0, 100), count: rows.length };
        }
        ...
        if (op === "previewTiempoAlmuerzo") {
            ...
            const rows = await queryTiempoAlmuerzoRows(reportDb, mf, orderKey);
            return { status: true, data: rows.slice(0, 100), count: rows.length };
        }
        ...
        if (op === "previewLoginMarca") {
            ...
            const rows = await queryLoginMarcaRows(reportDb, mf, orderKey);
            return { status: true, data: rows.slice(0, 100), count: rows.length };
        }
        ...
        if (op === "previewSolicitudesPermiso") {
            ...
            const rows = await querySolicitudesPermisoRows(reportDb, mf, orderKey);
            return { status: true, data: rows.slice(0, 100), count: rows.length };
        }
        ...
        if (op === "listReports") {
            ...
            let rows = await reportDb.e_reportes_mobile.findMany({
                where,
                orderBy: { id: "desc" },
                take: 300,
            });
            ...
            const data = await enrichReportRowsWithCreatorNames(reportDb, rows);
            return { status: true, data };
        }
        ...
        if (op === "createReportJob") {
            ...
            if (!modulo) {
                return { status: false, message: "modulo es obligatorio" };
            }
            ...
            const serviceAccessToken = resolveUserAccessToken(req, payload.token);
            const filtersObj = {
                moduleKey: modulo,
                moduleFilters,
        ...
        if (op === "mobileReportUploadsPath") {
            ...
            if (!Number.isFinite(reportId) || reportId <= 0) {
                return { status: false, message: "reportId es obligatorio" };
            }

            const row = await reportDb.e_reportes_mobile.findUnique({ where: { id: reportId } });
            if (!row) {
                return { status: false, message: "Reporte no encontrado" };
            }
            if (String(row.estado || "").toLowerCase() !== "completado") {
                return { status: false, message: "El reporte aún no está completado" };
            }

            const relativeUrl = resolveReportMobileRelativeUrl(reportId, row.filters);
            if (!relativeUrl) {
                return { status: false, message: "Archivo no encontrado en el servidor" };
            }
            return { status: true, data: { url: relativeUrl } };
        }

        return { status: false, message: `Operación no soportada: ${op}` };
    } catch (error: unknown) {
        const msg = error instanceof Error ? error.message : "Error desconocido";
        console.error("executeReportesOperation:", msg);
