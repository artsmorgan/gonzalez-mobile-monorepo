...
export async function GET(
  ...
  const activoId = parseInt(resolvedParams.id, 10);
  const fileName = resolvedParams.file;

  if (!activoId || !fileName) {
  ...
  const activo = await prisma.c_activo_mantenimiento.findUnique({ where: { id: activoId } });
  if (!activo) {
    return NextResponse.json(
  ...
  const fileRecord = await prisma.c_archivos_adjuntos_archivo_mantenimiento.findFirst({
    where: { activo_mantenimiento_id: activo.id, name: fileName },
  });
